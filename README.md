# Airbnb Listing Curator (Hiding/Highlighting)
Allows hiding or highlighting listings on Airbnb with persistent storage.

### Installing

1. Install a userscript manager: [Tampermonkey](https://www.tampermonkey.net/index.php?locale=en) or [Violentmonkey](https://violentmonkey.github.io/) supported by most browsers.
2. Click [here](https://github.com/Archer4499/Airbnb-Listing-Curator-Script/raw/main/airbnb_listing_curator.user.js) to install this script.

### Usage

Use the userscript manager extension icon to access various menu items:
- Open a dashboard menu that allows managing previously hidden or highlighted listings.
- Clear all the saved data.
